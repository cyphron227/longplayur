...
## Cache-busting: unhashed static assets can serve stale after a deploy (2026-08-10)

An automated review reported a Spotify "Invalid limit" console error already fixed once before (see `search.js`'s own comment on the `GET /artists/{id}/albums` limit-10 quirk) reappearing live. Checked every `limit=` parameter in the codebase against that fix -- all correctly bounded, nothing in the current source can produce that error. This app ships plain, unhashed filenames with no build step (per `Docs/CLAUDE.md`, it must also run unbuilt from `npx serve .` and deploy to GitHub Pages as-is), so a deploy never changes a URL the way fingerprinted assets normally would -- the most plausible explanation is a browser or Vercel's CDN serving a pre-fix copy of `js/search.js`/`js/newarrivals.js` under its default static-asset caching, not a live bug in what actually shipped.

Not independently reproducible from here (no way to inspect the reviewer's own cache state), so this is a plausible diagnosis, not a confirmed root cause. Added `vercel.json` setting `Cache-Control: public, max-age=0, must-revalidate` on `index.html`, `styles.css`, everything under `/js/`, and everything under `/bags/` -- this does not disable caching (a `304 Not Modified` still short-circuits an unchanged file efficiently), it just forces a real revalidation with the server on every load instead of trusting a cached copy for some TTL, so a shipped fix can never be masked by a stale asset again. Deliberately scoped to only the files that actually change across a deploy: fonts and the favicon are left on their default (long) caching since they're either genuinely static (fonts) or already covered by the pattern above (favicon). GitHub Pages ignores `vercel.json` entirely and already applies its own short default caching, so this is additive for the Vercel deployment specifically, not a behaviour change for the GitHub Pages path.

Considered and rejected: versioning every one of the ~43 first-party `import './x.js'` specifiers with a `?v=` query string, the alternative that doesn't rely on any hosting-specific config. Rejected because it is real, ongoing maintenance debt in a hand-authored, no-build codebase -- every future new import needs the same treatment, with no build step to enforce or automate it, and a missed one silently reintroduces exactly this bug. A hosting-level header is one file, applies automatically to files that don't exist yet, and cannot be forgotten on the next change.

## Screens rendering stacked instead of replacing each other: a systemic `[hidden]` bug, plus a Now Playing tracklist redesign (2026-08-10)

Reported live: opening a record bag "opens on the bottom half of the page instead of filling up the whole page", generally "trying to show multiple things on the page all the time". Rather than patch the one reported spot, this build environment's own Playwright/Chromium was used to actually load the app and screenshot it (mocking DOM/`hidden` state directly to skip the OAuth flow this environment can't complete) instead of reasoning about the CSS blind, per the explicit instruction not to keep guessing.

**Root cause, confirmed on screen, not just in the CSS source:** `.crates-body` sets `display: flex` and has no `[hidden]` override. Per this file's own already-documented rule (see the comment above `.past-sessions-list[hidden]` in `styles.css`, written after the *exact same bug* hit "By album" earlier), an author stylesheet's bare `display` always outranks the UA stylesheet's `[hidden]` rule, regardless of specificity. `main.js`'s `openBagDetail()`/`openBagBuilder()` call `hide(cratesBody)` expecting it to disappear; without the override it stayed rendered at full height, so the tab line/search/grid kept occupying the top of `#screen-crates` while bag detail or the bag builder rendered in whatever space was left underneath — two views stacked, not one replacing the other. Confirmed with a before/after screenshot (bag detail: chrome bleeding through above it, then cleanly filling the screen after the fix).

Given one instance of this bug class had already recurred once before (`.past-sessions-list`), every element this codebase ever calls `hide()`/`show()` on was audited systematically (a small script cross-referencing each one's CSS class against whether it sets `display` and whether a `[hidden]` companion rule exists), not just the one reported. Found three more real, confirmed-on-screen instances, none reported yet but all live bugs: `.app-tabs` (the full tab bar rendering over the Setup screen before a listener had even connected anything), `.setup-actions` (the "Connect Spotify" button and the "CONNECTED / Sign out" panel both showing at once on a fresh, never-connected load), and `.mode-toggle` (newly at risk from the Now Playing work below, caught before shipping rather than after). All four got their `[hidden]` companion rule; screenshots confirmed each one actually disappears now.

## Now Playing redesign: a focused tracklist view, not the Wall itself, once something is playing (2026-08-10)

Reported live, alongside the above: "the playing now page is still very focused on the album choosing content as opposed to the album being played and the tracks", and "you can't even see all the track names" at all -- correct, there was no tracklist UI anywhere in the app; `ceremony.js` only ever showed a track *count* in the deadwax line, never names. Requested flow: choose a record bag, choose an album within it, then land in a "now playing zone" focused on that album and its tracks -- not still looking at the browsable Wall.

Implemented as a third content area inside `#screen-app`, alongside the existing Spin (the dome) and Flip (searchable list): a Now Playing panel with cover, album/artist, and a full scrollable tracklist, tapping a track jumps straight to it (`playback.playTrackAtIndex()`, the same `PUT /me/player/play` call `commitPlayback()` already makes, just with a non-zero offset). It opens automatically the instant a needle drop commits (`handleNeedleDrop()`/`handleSelectAlbum()`'s own success path, after the ceremony's needle-drop animation has already fully settled -- it never competes with that animation for the screen) and is *not* a third Spin/Flip toggle button: Spin and Flip are both ways of browsing the same mounted pool, a genuine pair; Now Playing isn't a way of browsing at all. "Browse the wall instead" (deliberately reusing Runout groove's own escape-hatch copy) is the one way back to Spin, and explicitly choosing Spin or Flip from the mode toggle always wins over Now Playing being open, the same "browsing intent wins" rule used when switching bags. Selecting a *different* bag/playlist/search result while Now Playing is open (`switchWallPool()`) also closes it back to Spin, and does so *before* remounting the dome, not after -- DomeGallery measures its own container's visible size at mount time, so wall-viewport has to already be un-hidden by then, not hidden behind a Now Playing panel that hasn't closed yet.

The current track is kept in sync with the player bar's own progress (`playback.js`'s two view-model publishers now also expose `trackId`, previously computed internally for the deadwax elapsed-time math but never surfaced) and highlighted/scrolled into view on every tick, without rebuilding the list -- only a fresh needle drop rebuilds the actual rows.

**A second real bug, also only found by screenshotting it, not by reading the CSS:** the tracklist's own container had `flex: 1 1 auto; min-height: 0` but no `overflow` of its own, relying on the outer panel to scroll the whole column the way `.flip-view`/`.flip-list` do. A screenshot of a genuinely long (11-track) album showed "Browse the wall instead" rendered *on top of* a mid-list track row -- `overflow: visible` content doesn't get clipped or trigger scrolling, it just paints past its own box's bottom edge over whatever comes next in flow. Fixed by giving the tracklist its own `overflow-y: auto` (so the head and the "Browse" footer both stay genuinely fixed on screen, only the list itself scrolls) and moving the mobile fixed-player-bar clearance from the old (now non-scrolling) outer panel onto the footer that actually sits at the bottom of it. Re-screenshotted afterwards to confirm the fix, not just reasoned through.
...
Reported live. Root cause: this app never called the Web Media Session API (`navigator.mediaSession`) at all, at any point -- there was no code anywhere referencing it. That API is the *only* way a page tells the OS what is actually playing; without it, a Bluetooth speaker/car system (AVRCP), a phone lock screen, or a hardware media key has nothing to show beyond the page's own generic identity. Longplayur's audio is never in an `<audio>` element on this page to begin with (the Web Playback SDK renders it itself; Connect mode plays on a remote device) -- entirely orthogonal to that, and not something Media Session needs to know about. Fixed by setting `navigator.mediaSession.metadata` (title/artist/album/artwork) and `playbackState` on every player-bar update (`main.js`'s `updateMediaSessionMetadata()`, called from the same `updatePlayerBar()` that already drives the on-screen transport), and registering `play`/`pause`/`previoustrack`/`nexttrack` action handlers once at boot, routed to the exact same `playback.js` functions the on-screen buttons already call -- so a Bluetooth remote's or car's own transport controls work too, not just the metadata display. `play`/`pause` call `togglePlayPause()` with the state to move *away from* explicitly, rather than reusing the on-screen button's own toggle-current-state handler, since a hardware "play" press means "start playing" unconditionally, not "flip whatever it currently is" (a real difference if the two states ever drift out of sync).

### Credits: a second pass, still without live access

The three-bug fix from earlier still left credits "never found" in further live testing. Genuinely could not be diagnosed further against the real API from here -- this build environment's network egress does not reach `musicbrainz.org` at all (confirmed directly: the proxy rejects the `CONNECT`, and the same domain is explicitly blocked via the web-fetch tool too), so every change below is reasoned from what MusicBrainz's search API is documented to return and what could plausibly still be wrong, not confirmed against a live response.

- **The release-group search no longer trusts MusicBrainz's own combined relevance score as the only path to confidence.** The query joins two clauses (`AND artist:"..."`), and a score that comes from combining two Lucene field matches is not something this app has any way to calibrate without live access -- a release-group that is genuinely the right one could plausibly still score under `MIN_CONFIDENCE_SCORE` if the artist clause alone does not match as cleanly as the title clause does. Release-group search results carry their own credited artist(s) (`artist-credit`) independent of that score; this is now checked directly with the same `normalize()`-based comparison every other name match in this app already uses, and accepted as confirmation on its own, with the original score threshold kept only as a fallback for the (presumably rarer) case an `artist-credit` cannot be parsed. This is an additional, independent way to earn confidence, not a lowered bar -- a title match is still always required.
- **The title term is now sent unprefixed** (`"${title}" AND artist:"..."`) rather than field-prefixed (`releasegroup:"${title}"`). MusicBrainz's Lucene field name for a release-group's own title is believed to be `releasegroup`, but this cannot be confirmed here either; an unprefixed quoted phrase searches whatever MusicBrainz treats as this entity's default field, which for every MusicBrainz entity search is that entity's own name/title -- correct regardless of whether the specific field-prefixed name was right.
- **The disclosure now shows different copy for a genuine miss versus the lookup itself breaking** ("No credits found." vs "Could not check credits right now."), reversing an earlier explicit instruction to show identical copy either way. Revisited specifically because repeated live reports of "never found," with no way to reproduce or inspect the actual failure from here, made that distinction the only remaining way to tell "MusicBrainz genuinely has nothing for this one" from "something is still actually broken" without opening a browser console. The console-only diagnostic added in the previous pass (`console.info` with the top candidate, its score, and now its credited artist too, on every unconfident match) still stands alongside this.

Verified functionally with mocked MusicBrainz responses covering the specific scenario this pass targets (a candidate scoring under the old threshold but with a directly verifiable matching artist-credit) and a genuine miss, both behaving as intended -- not against the real API, for the reason stated above and in the previous entry.
