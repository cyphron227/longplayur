...
- A "Bags you've made" shelf sits above the six seed bags on the Record bags screen, its "+ Create a record bag" button opening a bag builder view in place of the shelf overview (the same in-place-swap pattern F11's bag detail view already uses). Name the bag, optionally give it a blurb, then search by album name or artist name (one free-text field, one query, no mode toggle -- Spotify's own album search already matches both) and tap covers to add or remove them; selections persist across further searches within the same bag.
...
- A custom bag behaves exactly like a seed bag everywhere else in the app (bag detail view, needle-dropping, journal `bagId` tagging, Runout groove, Flip), with one addition: its own detail view gets a "Delete this record bag" action, since -- unlike a seed bag or a Spotify playlist -- this is the one bag source the app itself owns and can remove.
