...
- Now Playing panel: a third content area on the Now Playing tab, alongside Spin (the dome) and Flip (the searchable list) — cover, album/artist, and a full scrollable tracklist (track number, name, duration), the currently playing track highlighted and kept in view as it advances. Opens automatically the moment a needle drop commits, so the flow is choose a record bag → choose an album → land focused on what's actually playing, not still looking at the Wall. Tapping a track jumps straight to it. Not a third Spin/Flip toggle choice — Spin and Flip are both ways of browsing the same pool; this isn't a way of browsing at all — "Browse the wall instead" (Runout groove's own escape-hatch copy, reused) is the one way back, and explicitly choosing Spin/Flip, or a different bag/playlist/search result, always closes it again.
