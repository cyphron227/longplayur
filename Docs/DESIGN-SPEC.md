...
Choosing what's on the Wall is its own screen/tab ("Record bags"), not a rail on the Wall itself, so browsing and search have room to breathe -- a single small entry-point button on the Wall (labelled with whatever source is currently loaded, "Your Record Bag" by default) opens it. One tab line along the top, not a stack of always-visible shelves: "Your record bags" (Your Record Bag itself, the user's own pool, is the first card in it, followed by New arrivals when there is one, the listener's own custom bags, then the six seed bags), "By mood", "By decade" and "Playlists" (the user's own Spotify playlists) each filter one wrapping grid below to just that set, rather than each permanently occupying its own row regardless of whether it's the one being browsed. The grid wraps rather than scrolling horizontally, so it uses width sensibly at any viewport -- a couple of columns on a phone, many more on a wide desktop window -- with no separate breakpoint needed for that part; the active tab persists between visits. Selecting anything crossfades the whole Wall (`--dur-breath`) to that source's own spiral layout and returns to the Now Playing tab; the camera always snaps to whole rows and columns on any switch, at any viewport, so no cover is ever cropped, letterboxing with obsidian where the aspect ratio does not divide evenly. A record bag or playlist is a curated/live list of albums, not a listening session: playing from one records normally into Past sessions, tagged with which bag or playlist it came from.
...
The "Your record bags" tab's own "+ Create a record bag" button opens a bag builder view in place of the tab's own grid (the same in-place swap the bag detail view already uses, not a new screen), wide enough from tablet width up that the name/blurb fields sit side by side rather than eating the vertical room the two album grids below them actually need. Name the bag, then search Spotify by album name or artist name -- one free-text field covers both, matching Spotify's own album search behaviour -- and tap covers to add them; an "In this bag" grid shows everything picked so far and stays visible (each cover removable from it) across as many further searches as it takes, not just a running count a fresh search would otherwise bury. Saving pins to the bottom of the view as a sticky footer, never requiring a scroll to the very end to reach on any viewport height. Saving drops the bag straight onto the "Your record bags" tab, resolved already (the albums are the exact Spotify releases the listener picked, not a title/artist pair resolved lazily like a seed bag). Its own detail view gets two further actions neither a seed bag nor a playlist has: "Add or remove albums" reopens the same bag builder pre-populated with the bag's current contents, saving over it in place rather than creating a new one; "Delete this record bag" removes it outright.
...
A mini now-playing strip (small art, track, artist) sits at the bottom of every screen other than Now Playing itself -- Record bags, Past sessions, Runout groove -- so browsing doesn't lose track of what's actually playing. It is an ordinary flex sibling of the screens, not an overlay: the visible screen simply shrinks to make room for it while something is playing and regains that room the moment nothing is, the same "no magic-number margin" flex behaviour the tab bar above the screens already relies on. Tapping it returns to Now Playing and resurfaces the hero cover, reusing the exact behaviour the full player bar's own small album art already triggers.
