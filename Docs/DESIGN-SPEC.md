...
A "Bags you've made" shelf sits above the six seed bags, its own "+ Create a record bag" button opening a bag builder view in place of the shelf overview (the same in-place swap the bag detail view already uses, not a new screen). Name the bag, then search Spotify by album name or artist name -- one free-text field covers both, matching Spotify's own album search behaviour -- and tap covers to add or remove them from the bag; selections persist across further searches. Saving drops the new bag straight onto its shelf, resolved already (the albums are the exact Spotify releases the listener picked, not a title/artist pair resolved lazily like a seed bag). Its own detail view gets a "Delete this record bag" action, the one bag source this app can actually remove.
