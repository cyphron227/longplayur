...
/**
 * Places rank 0 at the Wall's centre and each subsequent rank on the next
 * cell of an outward square spiral (ring 1 = ranks 1-8, ring 2 = ranks
 * 9-24, ring k = ranks (2k-1)^2 .. (2k+1)^2-1). Pure and deterministic.
 * @param {number} rank
 * @returns {{col: number, row: number}}
 */
export function spiralPosition(rank) {
  if (!Number.isInteger(rank) || rank < 0) {
    throw new RangeError('spiralPosition expects a non-negative integer rank');
  }
  if (rank === 0) return { col: 0, row: 0 };
