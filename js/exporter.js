...
import { sessionDurationMs } from './journal.js';
...
export async function renderSessionCard(session, sessionOrdinal) {
  ...
  const shownEntries = session.entries.slice(0, MAX_COVERS);
  ...
  drawStatsOverlay(ctx, session.entries.length, sessionDurationMs(session));
